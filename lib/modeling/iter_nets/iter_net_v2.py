...
class IterNetV2(nn.Module):
    def __init__(self, dim_in, dim_latent, n_iters=16):
    ...
    def get_input_encoding(self, x):
        ...
        logger.update(mean_grad=self.gaussian.mean.grad.abs().mean())
        logger.update(log_var_grad_mag=self.gaussian.log_var.grad.abs().mean())
        
        # concatenate all inputs
        x = (
            mean, log_var,
            mean.grad, log_var.grad
        )
        x = torch.cat(x, dim=1)
        return x
