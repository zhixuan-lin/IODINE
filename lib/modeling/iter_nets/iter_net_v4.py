...
class IterNetV4(nn.Module):
    ...
    def forward(self, x, n_samples=1, reduce=True):
        ...
        for i in range(self.n_iters):
            ...
            # debug
            # import time
            # print('iter: {}, elbo: {}, mean: {}'.format(i, elbo.mean().item(), self.gaussian.mean.mean()))
            # time.sleep(0.5)
...
class GaussianLayer(nn.Module):
    ...
    def forward(self, x, n_samples):
        ...
        self.mean = self.mean + self.mean_layer(x)
        # log standard deviation here
        self.log_var = self.log_var + self.log_var_layer(x)
